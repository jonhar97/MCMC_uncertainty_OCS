"""
Re-run Constrained OCS Using Existing Robustness Scores - Loblolly Pine
=======================================================================
VERSION: 2.0 - Excludes bottom 25% of SELECTED individuals by robustness

This script:
1. Loads existing robustness scores (already calculated)
2. Runs MAP-OCS on all 926 individuals
3. Identifies high-risk individuals based on robustness scores
...
OUTPUT_FILE = joinpath(DATA_PATH, "taeda_ocs_solutions_complete.csv")
...
# ============================================================================
# STEP 2: Load and Calculate MAP Breeding Values
# ============================================================================
...
ht6 = CSV.read(EBV_FILES["HT6"], DataFrame, delim=',', header=true, missingstring="NA")
dbh6 = CSV.read(EBV_FILES["DBH6"], DataFrame, delim=',', header=true, missingstring="NA")
gv6 = CSV.read(EBV_FILES["GV6"], DataFrame, delim=',', header=true, missingstring="NA")
wdn4 = CSV.read(EBV_FILES["WDN4"], DataFrame, delim=',', header=true, missingstring="NA")

println("✓ EBV files loaded")

# Calculate aggregate index: (HT6 + DBH6 + WDN4 - GV6) / 4
gav = (ht6[!, 2] .+ dbh6[!, 2] .+ wdn4[!, 2] .- gv6[!, 2]) ./ 4
...
map_selected_contrib = map_marginal[selected_indices]
const_selected_contrib = const_marginal[constrained_selected_indices]
...
# Create DataFrame with ALL 926 individuals
...
println("  All 926 individuals included")
...
println("\n" * "="^80)
println("SUMMARY - LOBLOLLY PINE DUAL METRICS")
println("="^80)
...
println("  Excluded: $(length(high_risk_selected))")
println("  Percentage of selected: $(round(length(high_risk_selected)/n_selected*100, digits=1))%")
...
if length(map_selected_robustness) > 0 && length(const_selected_robustness) > 0
    ...
    robustness_improvement = ((mean(const_selected_robustness) - mean(map_selected_robustness)) / 
                              abs(mean(map_selected_robustness))) * 100
    println("  MAP-OCS mean: $(round(mean(map_selected_robustness), digits=6))")
    println("  Constrained mean: $(round(mean(const_selected_robustness), digits=6))")
    println("  Improvement: $(round(robustness_improvement, digits=2))%")
end
...
println("="^80)
println("\nNext step:")
println("  Use visualize_dual_metrics_comprehensive_v2.jl with:")
println("  ROBUSTNESS_FILE = stable_robustness_results_926.csv")
println("  SOLUTIONS_FILE = taeda_ocs_solutions_complete.csv")
println("="^80)
