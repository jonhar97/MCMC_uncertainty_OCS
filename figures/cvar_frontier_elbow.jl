...
Usage (after running the CVaR optimisation script for your species):
...
# =============================================================================
# CONFIGURATION — set SPECIES here, everything else follows automatically
# =============================================================================
#   "spruce"  → Norway Spruce (Picea abies)
#   "qtlmas"  → QTLMAS 2010 simulation dataset
...
if SPECIES == "spruce"
    ...
    FRONTIER_FILE = joinpath(SAVE_DIR, "cvar_ocs_frontier_spruce.csv")
    EVAL_FILE     = ""   # spruce uses MAP in-sample gain directly from frontier
    OUTPUT_PDF    = joinpath(FIG_DIR,  "cvar_frontier_elbow_spruce.pdf")
    OUTPUT_PNG    = joinpath(FIG_DIR,  "cvar_frontier_elbow_spruce.png")
    METRICS_FILE  = joinpath(SAVE_DIR, "frontier_metrics_with_elbow.csv")

elseif SPECIES == "qtlmas"
    ...
    SAVE_DIR      = joinpath(BASE_DIR, "Save5")
    FIG_DIR       = joinpath(BASE_DIR, "Figures")
    FRONTIER_FILE = joinpath(SAVE_DIR, "cvar_ocs_frontier.csv")
    # EVAL_FILE provides in-sample MAP gain — needed because MAP-OCS gain_exp
    # is optimistic (vs posterior mean EBV), not in-sample (vs MCMC scenarios).
    # Using in-sample MAP gain as baseline gives a meaningful gain loss axis.
    EVAL_FILE     = joinpath(SAVE_DIR, "evaluation_gains_qtlmas.csv")
    OUTPUT_PDF    = joinpath(FIG_DIR,  "cvar_frontier_elbow_qtlmas.pdf")
    OUTPUT_PNG    = joinpath(FIG_DIR,  "cvar_frontier_elbow_qtlmas.png")
    METRICS_FILE  = joinpath(SAVE_DIR, "frontier_metrics_with_elbow.csv")

else
    error("SPECIES must be \"spruce\" or \"qtlmas\"")
end

# α levels to show (must match what the CVaR script produced)
...
function compute_frontier_metrics(frontier::DataFrame;
                                   map_gain_override::Union{Float64,Nothing}=nothing)
    map_row  = filter(r -> r.label == "MAP-OCS", frontier)[1, :]
    # For QTLMAS: MAP gain_exp is optimistic (vs posterior mean), not in-sample.
    # Use map_gain_override (mean of in-sample evaluation) as the correct baseline.
    map_gain = isnothing(map_gain_override) ? map_row.gain_exp : map_gain_override
    map_cvar = map_row.cvar95_eval
    if !isnothing(map_gain_override)
        println(@sprintf("    MAP gain baseline: %.4f (in-sample override; optimistic=%.4f, bias=%.1f%%)",
                map_gain, map_row.gain_exp,
                (map_row.gain_exp - map_gain) / map_row.gain_exp * 100))
    end
...
function run_elbow_analysis()
    ...
    # For QTLMAS: load in-sample evaluation gains to get unbiased MAP baseline.
    # MAP-OCS gain_exp is optimised vs posterior mean (optimistic); we use the
    # mean in-sample gain across MCMC scenarios as the correct gain loss axis.
    map_gain_override = nothing
    if SPECIES == "qtlmas" && EVAL_FILE != "" && isfile(EVAL_FILE)
        eval_df = CSV.read(EVAL_FILE, DataFrame)
        map_rows = filter(r -> r.solution == "MAP-OCS", eval_df)
        if nrow(map_rows) > 0
            map_gain_override = mean(map_rows.gain)
            println(@sprintf("    Loaded evaluation file: %d MAP-OCS scenarios", nrow(map_rows)))
        end
    end

    metrics, map_gain, map_cvar = compute_frontier_metrics(frontier;
                                      map_gain_override=map_gain_override)
...
To switch dataset, set SPECIES at the top of the script:
  SPECIES = "spruce"   → Norway Spruce
  SPECIES = "qtlmas"   → QTLMAS 2010 simulation
