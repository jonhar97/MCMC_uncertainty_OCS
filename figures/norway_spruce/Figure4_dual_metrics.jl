"""
Comprehensive Dual Metrics Visualization - Norway Spruce
========================================================
...
# Input files
ROBUSTNESS_FILE = "C:\\Users\\joah\\OneDrive - Skogforsk\\Documents\\Projekt\\Optimum contribution selection\\NorwaySpruceData\\Save\\robustness_analysis.csv"
SOLUTIONS_FILE = "C:\\Users\\joah\\OneDrive - Skogforsk\\Documents\\Projekt\\Optimum contribution selection\\NorwaySpruceData\\Save\\spruce_ocs_solutions_complete.csv"
# Output directory
OUTPUT_DIR = "C:\\Users\\joah\\OneDrive - Skogforsk\\Documents\\Projekt\\Optimum contribution selection\\NorwaySpruceData\\Save"
mkpath(OUTPUT_DIR)
...
# ============================================================================
# MERGE ROBUSTNESS WITH SOLUTIONS
# ============================================================================

# The robustness file has only analyzed individuals
# Need to merge with solutions file which has all individuals

println("\nMerging robustness scores with solutions...")

# Create a lookup for robustness scores
robustness_lookup = Dict(robustness_df.individual .=> robustness_df.robustness_score)

# Add robustness scores to solutions (NaN for non-analyzed individuals)
solutions_df.robustness_score = map(solutions_df.individual) do ind
    get(robustness_lookup, ind, NaN)
end

# Add breeding values from robustness df
breeding_value_lookup = Dict(robustness_df.individual .=> robustness_df.breeding_value)
solutions_df.breeding_value = map(solutions_df.individual) do ind
    get(breeding_value_lookup, ind, 0.0)
end

# Filter to only individuals with robustness scores for panels A & B
analyzed_df = filter(row -> !isnan(row.robustness_score), solutions_df)

println("✓ Data merged")
println("  Individuals with robustness scores: $(nrow(analyzed_df))")
...
# MAP-OCS metrics (using map_marginal_contrib from your file)
map_selected = solutions_df.map_contribution .> threshold
map_contributions = solutions_df.map_marginal_contrib[map_selected]
map_contributions = map_contributions[.!isnan.(map_contributions)]  # Remove NaN

map_mean = mean(abs.(map_contributions))
map_max = maximum(abs.(map_contributions))
map_conc = sum(sort(abs.(map_contributions), rev=true)[1:min(Int(ceil(length(map_contributions)*0.1)), length(map_contributions))]) / sum(abs.(map_contributions)) * 100

# Gini coefficient for MAP
sorted_map = sort(abs.(map_contributions))
n_map = length(sorted_map)
map_gini = sum((2 .* (1:n_map) .- n_map .- 1) .* sorted_map) / (n_map * sum(sorted_map))

# Constrained OCS metrics (using constrained_marginal_contrib from your file)
const_selected = solutions_df.constrained_contribution .> threshold
const_contributions = solutions_df.constrained_marginal_contrib[const_selected]
const_contributions = const_contributions[.!isnan.(const_contributions)]  # Remove NaN

const_mean = mean(abs.(const_contributions))
const_max = maximum(abs.(const_contributions))
const_conc = sum(sort(abs.(const_contributions), rev=true)[1:min(Int(ceil(length(const_contributions)*0.1)), length(const_contributions))]) / sum(abs.(const_contributions)) * 100

# Gini coefficient for Constrained
sorted_const = sort(abs.(const_contributions))
...
println("  MAP-OCS: $(sum(map_selected)) selected")
println("  Constrained OCS: $(sum(const_selected)) selected")

# ============================================================================
# DETERMINE SELECTION STATUS
# ============================================================================

# Add selection status to analyzed_df
analyzed_df.selected_in_map = analyzed_df.map_contribution .> threshold
...
sort!(analyzed_df, :breeding_value, rev=true)

# Quartiles for robustness
q25 = quantile(analyzed_df.robustness_score, 0.25)
q50 = quantile(analyzed_df.robustness_score, 0.50)
q75 = quantile(analyzed_df.robustness_score, 0.75)
...
selected_mask = analyzed_df.selected_in_map
unselected_mask = .!analyzed_df.selected_in_map
x_pos = 1:nrow(analyzed_df)

# Unselected (gray)
if sum(unselected_mask) > 0
    bar!(p[1], x_pos[unselected_mask], analyzed_df.robustness_score[unselected_mask],
...
if sum(selected_mask) > 0
    bar!(p[1], x_pos[selected_mask], analyzed_df.robustness_score[selected_mask],
         color=:darkblue, label="Selected by MAP-OCS", alpha=1.0, linewidth=0, linecolor=:match)
end

# Reference lines
hline!(p[1], [mean(analyzed_df.robustness_score)], 
...
# Assign quartiles
analyzed_df.quartile = map(analyzed_df.robustness_score) do score
    if score < q25
        "Q1"
    elseif score < q50
        "Q2"
...
for q in ["Q1", "Q2", "Q3", "Q4"]
    mask = analyzed_df.quartile .== q
    if sum(mask) > 0
        for (i, idx) in enumerate(findall(mask))
            marker_shape = analyzed_df.selected_in_map[idx] ? :circle : :xcross
            scatter!(p[2], [idx], [analyzed_df.robustness_score[idx]],
...
# Mean line
hline!(p[2], [mean(analyzed_df.robustness_score)], 
...
plot!(p[3], xticks=(x_metrics, metric_names),
      ...
      legend=:topleft,  # Changed from :topright to :bottomleft
      legendfontsize=8)
...
map_data = DataFrame(
    contribution = abs.(map_contributions),
    solution = fill("MAP-OCS", length(map_contributions))
)

const_data = DataFrame(
    contribution = abs.(const_contributions),
...
const_stats = @sprintf("Constrained\nn=%d\nμ=%.4f\nσ=%.4f", 
                       ...
                       mean(abs.(const_contributions)), 
                       std(abs.(const_contributions)))
map_stats = @sprintf("MAP-OCS\nn=%d\nμ=%.4f\nσ=%.4f", 
                     length(map_contributions), 
                     mean(abs.(map_contributions)), 
                     std(abs.(map_contributions)))

# Place text to the LEFT of each violin at mid-height
y_mid = maximum(combined_data.contribution) * 0.8
...
# Save
output_file = joinpath(OUTPUT_DIR, "dual_metrics_comprehensive_figure_spruce.pdf")
savefig(p, output_file)
println("✓ Saved: $output_file")

output_file_png = joinpath(OUTPUT_DIR, "dual_metrics_comprehensive_figure_spruce.png")
...
println("\n" * "="^80)
println("DUAL METRICS SUMMARY - NORWAY SPRUCE")
println("="^80)

println("\nMETRIC 1: Individual Importance (MCMC-Based)")
selected_rob = analyzed_df.robustness_score[analyzed_df.selected_in_map]
unselected_rob = analyzed_df.robustness_score[.!analyzed_df.selected_in_map]

println("  Selected individuals (n=$(sum(analyzed_df.selected_in_map))):")
...
if length(unselected_rob) > 0
    println("  Unselected individuals (n=$(sum(.!analyzed_df.selected_in_map))):")
...
println("    MAP-OCS: $(round(map_max, digits=6)) ($(round(map_max/sum(abs.(map_contributions))*100, digits=2))% of total)")
println("    Constrained: $(round(const_max, digits=6)) ($(round(const_max/sum(abs.(const_contributions))*100, digits=2))% of total)")
