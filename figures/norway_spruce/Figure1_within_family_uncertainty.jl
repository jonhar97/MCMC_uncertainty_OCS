...
MCMC_HJD17_FILE = joinpath(RESULTS_PATH, "MCMC_samples_EBV_Hjd17.txt")
MCMC_HTV17_FILE = joinpath(RESULTS_PATH, "MCMC_samples_EBV_Htv17.txt")
MCMC_SPRANT17_FILE = joinpath(RESULTS_PATH, "MCMC_samples_EBV_Sprant17.txt")

# Selection index formula: I = (Hjd17 + Htv17 - Sprant17) / 3
INDEX_FORMULA = (hjd, htv, sprant) -> (hjd .+ htv .- sprant) ./ 3

# JWAS phenotypes file with family information
PHENOTYPES_FILE = joinpath(SAVE_PATH, "JWAS_phenotypes_1218.txt")
FAMILY_COLUMN = "Famly"  # Column name in phenotypes file
...
SELECTED_FAMILIES = [55353, 55385, 55389]  # High, Medium, Low performing
...
# Output
OUTPUT_FILE = joinpath(SAVE_PATH, "Figure1.pdf")
...
"""
function load_selection_index_mcmc(hjd_file::String, htv_file::String, 
                                   sprant_file::String, index_formula)
    println("\nLoading MCMC samples for selection index I = (Hjd17 + Htv17 - Sprant17) / 3")
    println("="^80)
    
    # Load each trait
    hjd_samples = load_trait_mcmc(hjd_file, "Hjd17")
    htv_samples = load_trait_mcmc(htv_file, "Htv17")
    sprant_samples = load_trait_mcmc(sprant_file, "Sprant17")
    
    # Verify dimensions match
    if size(hjd_samples) != size(htv_samples) || size(hjd_samples) != size(sprant_samples)
        error("Dimension mismatch between trait files!")
    end
    
    n_individuals = size(hjd_samples, 1)
    n_iterations = size(hjd_samples, 2)
    
    println("\n  Computing selection index...")
    
    # Compute selection index for each individual × iteration
    index_samples = index_formula(hjd_samples, htv_samples, sprant_samples)
...
"""
function get_family_individuals(family_ids::Vector, target_family::Int)
...
function plot_family_panel!(p, mcmc_samples::DataFrame, family_ids::Vector,
                           family_id::Int, family_label::String,
                           posterior_means::Vector{Float64},
...
DATA STRUCTURE:
✓ Three trait MCMC files: Hjd17, Htv17, Sprant17 (each 1218 × 1000)
✓ Selection index: I = (Hjd17 + Htv17 - Sprant17) / 3
✓ Pedigree file with family IDs (sire column)

PATHS CONFIGURED:
✓ MCMC samples: results_JWAS_1218_G_adj_Lev17/MCMC_samples_EBV_*.txt
✓ Pedigree: ped_Pabi_1218.txt

SELECTED FAMILIES (update if needed):
✓ High-performing: Family 55353
✓ Medium-performing: Family 55385  
✓ Low-performing: Family 55389

USAGE:
1. Verify your pedigree file path is correct
2. Update SELECTED_FAMILIES if you want different examples
3. Run: include("Figure1_within_family_uncertainty.jl")
        results = main()
...
OUTPUT FILES:
✓ Figure1_publication.pdf (vector graphics)
✓ Figure1_publication.png (raster for preview)

FIGURE CAPTION:
"Posterior uncertainty in estimated breeding values reveals ranking ambiguity 
within Norway spruce families. Kernel density plots show the posterior 
distributions of selection index breeding values (Hjd17 + Htv17 - Sprant17) 
for individuals within three representative families: high-performing 
(Family 55353, n=26), medium-performing (Family 55385, n=10), and 
low-performing (Family 55389, n=23). Each thin curve represents the posterior 
distribution for one individual based on 1000 MCMC samples. Thick curves 
highlight the top 5 individuals within each family ranked by posterior mean. 
Dashed vertical lines indicate family means."
